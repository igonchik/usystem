...
class FileManager(object):
    ...
    def handle_form(self, form, files):
        ...
        if action == 'upload':
            ...
            if platform.system() == 'Windows':
                path = path.replace('/', '\\')
            path = path[:-1]
            path = os.path.dirname(self.basepath + path)
            try:
                os.chdir(path)
            ...
            if old_ext == new_ext:
                ...
                try:
                    os.chdir(os.path.dirname(self.basepath + path))
                    os.rename(oldname, name)
                    messages.append(
            ...
            if self.current_path.find(path) == 0:
                ...
                filename = self.basepath + self.current_path
                filename = os.path.join(filename, os.path.basename(path))
    ...
    def media(self, path):
        ...
        try:
            ...
            img = Image.open(os.path.join(self.basepath, path))
            ...
            imagepath = (os.path.join(settings.FILEMANAGER_STATIC_ROOT, 'images', 'icons', ext+'.png'))
            if not os.path.exists(imagepath):
                imagepath = (
                        os.path.join(settings.FILEMANAGER_STATIC_ROOT, 'images', 'icons', 'default.png')
                )
