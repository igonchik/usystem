...
function SubmitFile(filename, path) {
    $('.dialog-content').html(progress);
    $('.js-send-file').addClass('notshow');
    $.ajax({
        url: '/sendfile/'+'?filename='+encodeURIComponent(filename)+'&path='+path,
        success: function(data) {
            $('.dialog-content').html('<p>Успех!</p>');
        },
        error: function() {
...
function onDrawRow1_ (tr) {
    ...
    var id_col = tr.find('.id_col')[0].innerHTML;
    $(tr.find('td')[0]).html('<input type="checkbox" data-role="checkbox" class="forsend_check" value="'+id_col+'">');
}
...
function size(){
    var top = 25;
    $('#main').height($(window).height()-47-top);
    $('#left').height($(window).height()-69-top);
    $('#right').height($(window).height()-49-top);
    $('#content').height($(window).height()-100-top-5);
}
...
function onload(){
    ...
    $('.dialog-content').bind('click',function(e){$('#dir-menu').hide();$('.unzip-menu').hide();$('#file-menu').hide();});
        ...
        content: function() {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('.js-send-file').removeClass('notshow');
                    $('.js-send-file').click(function () {
                        var agents_tosend = '';
                        $('.dialog-content input[type=checkbox]').each(function () {
                            if (this.checked) {
                                agents_tosend = agents_tosend + $(this).val() + ',';
                            }
                        });
                        SubmitFile(filename, agents_tosend);
                    });
                },
                error: function() {
                    $('.dialog-content').html(progresserror);
...
{var heading;
    ...
    {if(act == 'rename')heading = "Enter new name of file";
        if(act == 'delete'){form_submit(act,t);return;}
        if(act == 'sendto'){sendToAgent(get_path(dir_id)+selected_file);return;}
