...
.. _Issue #29537: https://github.com/saltstack/usystem/issues/29537
...
``/etc/usystem/cloud.providers`` or ``/etc/usystem/cloud.providers.d/vmware.conf``:
...
def test_vcenter_connection(kwargs=None, call=None):
    ...
    the specified credentials inside ``/etc/usystem/cloud.providers``
    or ``/etc/usystem/cloud.providers.d/vmware.conf``
...
def list_nodes_select(call=None):
    ...
    specified under ``query.selection`` in ``/etc/usystem/cloud``
    ...
    fields specified under ``query.selection`` in ``/etc/usystem/cloud``:
    ...
    if not selection:
        raise SaltCloudSystemExit(
            'query.selection not found in /etc/usystem/cloud'
        )
