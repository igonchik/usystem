...
install_ubuntu_stable_deps() {
    if [ "${_SLEEP}" -eq "${__DEFAULT_SLEEP}" ] && [ "$DISTRO_MAJOR_VERSION" -lt 16 ]; then
        ...
        echodebug "See https://github.com/saltstack/usystem/issues/12248 for more info."
        _SLEEP=10
    fi
...
install_ubuntu_git() {
    ...
    if [ -f "${_SALT_GIT_CHECKOUT_DIR}/usystem/syspaths.py" ]; then
    ...
    if [ "$_TEMP_CONFIG_DIR" = "null" ]; then
        ...
        if [ ! -f "$_SALT_ETC_DIR/minion" ] && [ ! -f "$_TEMP_CONFIG_DIR/minion" ]; then
            ...
            curl $_CURL_ARGS -s -o "$_TEMP_CONFIG_DIR/minion" -L \
                https://raw.githubusercontent.com/saltstack/usystem/develop/conf/minion || return 1
        fi
        if [ ! -f "$_SALT_ETC_DIR/master" ] && [ ! -f $_TEMP_CONFIG_DIR/master ]; then
            # shellcheck disable=SC2086
            curl $_CURL_ARGS -s -o "$_TEMP_CONFIG_DIR/master" -L \
                https://raw.githubusercontent.com/saltstack/usystem/develop/conf/master || return 1
        fi
    fi
        ...
        if [ $? -eq 1 ]; then
            if [ ! -f "$_TEMP_CONFIG_DIR/salt-$fname.xml" ]; then
                ...
                curl $_CURL_ARGS -s -o "$_TEMP_CONFIG_DIR/salt-$fname.xml" -L \
                    "https://raw.githubusercontent.com/saltstack/usystem/develop/pkg/smartos/salt-$fname.xml"
            fi
...
if [ "$DAEMONS_RUNNING_FUNC" != "null" ] && [ ${_START_DAEMONS} -eq $BS_TRUE ]; then
    ...
    if [ $? -ne 0 ]; then
        ...
        for fname in api master minion syndic; do
            ...
            [ ! -f /var/log/usystem/$fname ] && echodebug "/var/log/usystem/$fname does not exist. Can't cat its contents!" && continue

            echodebug "DAEMON LOGS for $fname:"
            echodebug "$(cat /var/log/usystem/$fname)"
            echo
        done
