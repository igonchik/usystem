...
def list_(show_all=False,
    ...
    try:
        ...
        if res:
            event_ret = eventer.get_event(tag='/usystem/minion/minion_schedule_list_complete', wait=30)
    ...
    for name in list_(show_all=True, return_yaml=False):
        ...
        if 'test' in kwargs and kwargs['test']:
            ...
            try:
                ...
                if res:
                    event_ret = eventer.get_event(tag='/usystem/minion/minion_schedule_delete_complete', wait=30)
                    if event_ret and event_ret['complete']:
    ...
    if 'test' in kwargs and kwargs['test']:
        ...
        try:
            ...
            res = __salt__['event.fire']({'name': name,
                                          ...
                                          'persist': persist}, 'manage_schedule')
            if res:
                event_ret = eventer.get_event(tag='/usystem/minion/minion_schedule_add_complete', wait=30)
                if event_ret and event_ret['complete']:
                    schedule = event_ret['schedule']
                    if name in schedule:
                        ret['result'] = True
...
def enable_job(name, **kwargs):
    ...
    if 'test' in __opts__ and __opts__['test']:
        ...
        try:
            ...
            res = __salt__['event.fire'](event_data, 'manage_schedule')
            if res:
                event_ret = eventer.get_event(tag='/usystem/minion/minion_schedule_enabled_job_complete', wait=30)
                if event_ret and event_ret['complete']:
                    schedule = event_ret['schedule']
...
def disable_job(name, **kwargs):
    ...
    if 'test' in kwargs and kwargs['test']:
        ...
        try:
            ...
            res = __salt__['event.fire'](event_data, 'manage_schedule')
            if res:
                event_ret = eventer.get_event(tag='/usystem/minion/minion_schedule_disabled_job_complete', wait=30)
                if event_ret and event_ret['complete']:
                    schedule = event_ret['schedule']
                    # check item exists in schedule and is enabled
...
def save(**kwargs):
    ...
    if 'test' in kwargs and kwargs['test']:
        ...
        try:
            ...
            if res:
                event_ret = eventer.get_event(tag='/usystem/minion/minion_schedule_saved', wait=30)
                if event_ret and event_ret['complete']:
                    ret['result'] = True
...
def enable(**kwargs):
    ...
    if 'test' in kwargs and kwargs['test']:
        ...
        try:
            ...
            if res:
                event_ret = eventer.get_event(tag='/usystem/minion/minion_schedule_enabled_complete', wait=30)
                if event_ret and event_ret['complete']:
                    schedule = event_ret['schedule']
...
def disable(**kwargs):
    ...
    if 'test' in kwargs and kwargs['test']:
        ...
        try:
            ...
            if res:
                event_ret = eventer.get_event(tag='/usystem/minion/minion_schedule_disabled_complete', wait=30)
                if event_ret and event_ret['complete']:
                    schedule = event_ret['schedule']
...
def postpone_job(name,
    ...
    if 'test' in __opts__ and __opts__['test']:
        ...
        try:
            ...
            res = __salt__['event.fire'](event_data, 'manage_schedule')
            if res:
                event_ret = eventer.get_event(tag='/usystem/minion/minion_schedule_postpone_job_complete', wait=30)
                if event_ret and event_ret['complete']:
                    schedule = event_ret['schedule']
                    # check item exists in schedule and is enabled
                    if name in schedule and schedule[name]['enabled']:
...
def skip_job(name, current_time, **kwargs):
    ...
    if 'test' in __opts__ and __opts__['test']:
        ...
        try:
            ...
            res = __salt__['event.fire'](event_data, 'manage_schedule')
            if res:
                event_ret = eventer.get_event(tag='/usystem/minion/minion_schedule_skip_job_complete', wait=30)
                if event_ret and event_ret['complete']:
                    schedule = event_ret['schedule']
                    # check item exists in schedule and is enabled
                    if name in schedule and schedule[name]['enabled']:
...
def show_next_fire_time(name, **kwargs):
    ...
    try:
        ...
        res = __salt__['event.fire'](event_data,
                                     'manage_schedule')
        if res:
            event_ret = eventer.get_event(tag='/usystem/minion/minion_schedule_next_fire_time_complete', wait=30)
    except KeyError:
        # Effectively a no-op, since we can't really return without an event system
        ret = {}
