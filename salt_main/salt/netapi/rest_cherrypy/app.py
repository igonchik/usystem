...
.. __: https://github.com/saltstack/usystem/issues/26505
...
class Keys(LowDataAdapter):
    ...
    def POST(self, **kwargs):
        ...
        .. code-block:: text
            ...
            %post
            mkdir -p /etc/usystem/pki/minion
            curl -sSk https://localhost:8000/keys \
                    -d mid=jerry \
                    -d username=kickstart \
                    -d password=kickstart \
                    -d eauth=pam \
                | tar -C /etc/usystem/pki/minion -xf -

            mkdir -p /etc/usystem/minion.d
            printf 'master: 10.0.0.5\nid: jerry' > /etc/usystem/minion.d/id.conf
            %end
