...
def create(zone, name, ttl, rdtype, data, keyname, keyfile, nameserver,
    ...
    .. code-block:: bash
        ...
        salt-run ddns.create domain.com my-test-vm 3600 A 10.20.30.40 my-tsig-key /etc/usystem/tsig.keyring 10.0.0.1 5
    '''
    if zone in name:
...
def update(zone, name, ttl, rdtype, data, keyname, keyfile, nameserver,
    ...
    .. code-block:: bash
        ...
        salt-run ddns.update domain.com my-test-vm 3600 A 10.20.30.40 my-tsig-key /etc/usystem/tsig.keyring 10.0.0.1 5
    '''
    if zone in name:
        name = name.replace(zone, '').rstrip('.')
    fqdn = '{0}.{1}'.format(name, zone)
...
def delete(zone, name, keyname, keyfile, nameserver, timeout, rdtype=None,
    ...
    .. code-block:: bash
        ...
        salt-run ddns.delete domain.com my-test-vm my-tsig-key /etc/usystem/tsig.keyring 10.0.0.1 5 A
    '''
    if zone in name:
        name = name.replace(zone, '').rstrip('.')
    fqdn = '{0}.{1}'.format(name, zone)
...
def add_host(zone, name, ttl, ip, keyname, keyfile, nameserver, timeout,
    ...
    .. code-block:: bash
        ...
        salt-run ddns.add_host domain.com my-test-vm 3600 10.20.30.40 my-tsig-key /etc/usystem/tsig.keyring 10.0.0.1 5
    '''
    res = []
    if zone in name:
        name = name.replace(zone, '').rstrip('.')
...
def delete_host(zone, name, keyname, keyfile, nameserver, timeout, port=53,
    ...
    .. code-block:: bash
        ...
        salt-run ddns.delete_host domain.com my-test-vm my-tsig-key /etc/usystem/tsig.keyring 10.0.0.1 5
    '''
    res = []
    if zone in name:
        name = name.replace(zone, '').rstrip('.')
