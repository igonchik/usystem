# -*- coding: utf-8 -*-
#GONCH comment path salt -> usystem
'''
...
if 'SETUP_DIRNAME' in globals():
    ...
    __THIS_FILE = os.path.join(SETUP_DIRNAME, 'usystem', 'syspaths.py')  # pylint: disable=E0602
else:
...
if CONFIG_DIR is None:
    if __PLATFORM.startswith('win'):
        ...
        CONFIG_DIR = os.path.join(ROOT_DIR, 'usr', 'local', 'etc', 'usystem')
    elif 'netbsd' in __PLATFORM:
        CONFIG_DIR = os.path.join(ROOT_DIR, 'usr', 'pkg', 'etc', 'usystem')
    elif 'sunos5' in __PLATFORM:
        CONFIG_DIR = os.path.join(ROOT_DIR, 'opt', 'local', 'etc', 'usystem')
    else:
        CONFIG_DIR = os.path.join(ROOT_DIR, 'etc', 'usystem')
...
if SHARE_DIR is None:
    if __PLATFORM.startswith('win'):
        ...
        SHARE_DIR = os.path.join(ROOT_DIR, 'usr', 'local', 'share', 'usystem')
    elif 'netbsd' in __PLATFORM:
        SHARE_DIR = os.path.join(ROOT_DIR, 'usr', 'share', 'usystem')
    elif 'sunos5' in __PLATFORM:
        SHARE_DIR = os.path.join(ROOT_DIR, 'usr', 'share', 'usystem')
    else:
        SHARE_DIR = os.path.join(ROOT_DIR, 'usr', 'share', 'usystem')
...
if CACHE_DIR is None:
    CACHE_DIR = os.path.join(ROOT_DIR, 'var', 'cache', 'usystem')

SOCK_DIR = __generated_syspaths.SOCK_DIR
if SOCK_DIR is None:
    SOCK_DIR = os.path.join(ROOT_DIR, 'var', 'run', 'usystem')
...
if BASE_FILE_ROOTS_DIR is None:
    BASE_FILE_ROOTS_DIR = os.path.join(SRV_ROOT_DIR, 'usystem')
...
if BASE_MASTER_ROOTS_DIR is None:
    BASE_MASTER_ROOTS_DIR = os.path.join(SRV_ROOT_DIR, 'usystem-master')

LOGS_DIR = __generated_syspaths.LOGS_DIR
if LOGS_DIR is None:
    LOGS_DIR = os.path.join(ROOT_DIR, 'var', 'log', 'usystem')
...
if SPM_FORMULA_PATH is None:
    SPM_FORMULA_PATH = os.path.join(SPM_PARENT_PATH, 'usystem')
